...
import type { AutoModelConfig, ProcessingSummary, ConfigObject, ResolvedModel } from "../types"

const DEFAULT_DUMP_PATH = path.join(os.homedir(), ".opencode", "expanded-config.json")
...
export class ConfigDumper {
  ...
  async dump(
    ...
    await fs.writeFile(this.dumpPath, JSON.stringify(content, null, 2), "utf-8")
    ...
    for (const [providerKey, models] of resolvedModels) {
      ...
      for (const [modelId, resolved] of models) {
        ...
        if (resolved.warning) {
          ...
          // 仅包含实际填充的字段
          const configFields = currentProviders[providerKey]?.models?.[modelId] || {}
